...
bool SfkToKey( sf::Key::Code code, Lib::Key key )
{
    ...
    if ( ( code == sf::Key::D || code == sf::Key::Right ) && key == Lib::KEY_RIGHT )
        return true;
    if ( ( code == sf::Key::Escape || code == sf::Key::Return ) && key == Lib::KEY_MENU )
        return true;
    if ( ( code == sf::Key::Z || code == sf::Key::LControl || code == sf::Key::RControl ) && key == Lib::KEY_FIRE )
        return true;
    if ( ( code == sf::Key::X || code == sf::Key::Space ) && key == Lib::KEY_BOMB )
        return true;
    if ( ( code == sf::Key::Z || code == sf::Key::Space || code == sf::Key::LControl || code == sf::Key::RControl ) && key == Lib::KEY_ACCEPT )
        return true;
    if ( ( code == sf::Key::X || code == sf::Key::Escape ) && key == Lib::KEY_CANCEL )
        return true;
    return false;
}
...
bool LibWin::Handler::axisMoved( const OIS::JoyStickEvent& arg, int axis )
{
    ...
    if ( p < 0 )
        return true;
    fixed v = std::max( -M_ONE, std::min( M_ONE, fixed( arg.state.mAxes[ axis ].abs ) / OIS::JoyStick::MAX_AXIS ) );
...
, _mouseMoving( true )
{
    ...
    DWORD result = GetModuleFileName( 0, &_exe[0], static_cast< DWORD >( _exe.size() ) );
    ...
    for ( int i = 0; i < _padCount; ++i ) {
        try {
            tpads[ i ] = ( OIS::JoyStick* )_manager->createInputObject( OIS::OISJoyStick, true );
    ...
    for ( int i = 0; i < PLAYERS; ++i ) {
        if ( !_pads[ i ] )
            ...
            _ff[ i ] = 0;
        _ff[ i ] = ( OIS::ForceFeedback* )_pads[ i ]->queryInterface( OIS::Interface::ForceFeedback );
        /*if ( _ff[ i ] ) {
            OIS::Effect* e = new OIS::Effect( OIS::Effect::ConstantForce, OIS::Effect::Constant );
            e->setNumAxes( 1 );
    ...
    for ( int i = int( sf::VideoMode::GetModesCount() ) - 1; i >= 0 && !LoadSaveSettings()._windowed; --i ) {
        sf::VideoMode m = sf::VideoMode::GetMode( i );
        if ( m.Width >= unsigned( Lib::WIDTH ) && m.Height >= unsigned( Lib::HEIGHT ) && m.BitsPerPixel == 32 ) {
            _window.Create( m, "WiiSPACE", sf::Style::Fullscreen, sf::WindowSettings( 0, 0, 0 ) );
            _extraX = ( m.Width - Lib::WIDTH ) / 2;
            _extraY = ( m.Height - Lib::HEIGHT ) / 2;
            return;
        }
    }
    _window.Create( sf::VideoMode( 640, 480, 32 ), "WiiSPACE", sf::Style::Close | sf::Style::Titlebar );
    _extraX = 0;
    _extraY = 0;
}
...
void LibWin::Init()
{
    ...
    sf::Listener::SetGlobalVolume( std::max( 0.f, std::min( 100.f, z_float( _settings._volume ) ) ) );
    for ( int i = 0; i < SOUND_MAX; ++i )
...
LibWin::PadType LibWin::IsPadConnected( int player ) const
{
    ...
    if ( player < _padCount )
        r = PAD_GAMEPAD;
    if ( ( GetPlayerCount() <= _padCount && player == GetPlayerCount() - 1 ) || ( GetPlayerCount() > _padCount && player == _padCount ) )
...
bool LibWin::IsKeyHeld( int player, Key k ) const
{
    ...
    if ( k == KEY_FIRE && ( _padAimDpads[ player ] != OIS::Pov::Centered || v.Length() >= M_PT_ONE * 2 ) )
        return true;
...
{
    bool kU = IsKeyHeld( player, KEY_UP ) || _padMoveDpads[ player ] & OIS::Pov::North;
    bool kD = IsKeyHeld( player, KEY_DOWN ) || _padMoveDpads[ player ] & OIS::Pov::South;
    bool kL = IsKeyHeld( player, KEY_LEFT ) || _padMoveDpads[ player ] & OIS::Pov::West;
    bool kR = IsKeyHeld( player, KEY_RIGHT ) || _padMoveDpads[ player ] & OIS::Pov::East;
...
{
    bool kp = player == ( GetPlayerCount() <= _padCount ? GetPlayerCount() - 1 : _padCount );
    Vec2 v( _padAimHAxes[ player ], _padAimVAxes[ player ] );
    if ( v.Length() >= M_PT_ONE * 2 ) {
        v.Normalise();
        v *= 48;
...
void LibWin::RenderText( const Vec2f& v, const std::string& text, Colour c ) const
{
    ...
    for ( std::size_t i = 0; i < text.length(); ++i ) {
        _font.SetPosition( ( int( i ) + v._x ) * TEXT_WIDTH + _extraX, v._y * TEXT_HEIGHT + _extraY );
        _font.SetSubRect( sf::IntRect( TEXT_WIDTH * text[ i ], 0, TEXT_WIDTH * ( 1 + text[ i ] ), TEXT_HEIGHT ) );
        _window.Draw( _font );
    }
    _window.Draw( sf::Shape::Rectangle( 0, 0, 0, 0, sf::Color() ) );
}

void LibWin::RenderRect( const Vec2f& low, const Vec2f& hi, Colour c, int lineWidth ) const
{
    c = Cycle( c );
    ...
    for ( std::size_t i = 0; i < 4; ++i ) {
        ...
        normals[ i ] = ( n1 + n2 ) / f;
        float dot = ( v1._x - centre._x ) * normals[ i ]._x + ( v1._y - centre._y ) * normals[ i ]._y;
        if ( dot < 0 )
    ...
    for ( std::size_t i = 0; i < 5; ++i )
    {
        ...
        glVertex3f( list[ i % 4 ]->_x + normals[ i % 4 ]._x, list[ i % 4 ]->_y + normals[ i % 4 ]._y, 0 );
    }
    glEnd();
    if ( lineWidth > 1 )
        RenderRect( low + Vec2f( 1.f, 1.f ), hi - Vec2f( 1.f, 1.f ), Cycle( c ), lineWidth - 1 );
}

void LibWin::Render() const
{
    _window.Draw( sf::Shape::Rectangle( 0.f, 0.f, float( _extraX ), float( Lib::HEIGHT + _extraY * 2 ), sf::Color( 0, 0, 0, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( 0.f, 0.f, float( Lib::WIDTH + _extraX * 2 ), float( _extraY ), sf::Color( 0, 0, 0, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( float( Lib::WIDTH + _extraX ), 0, float( Lib::WIDTH + _extraX * 2 ), float( Lib::HEIGHT + _extraY * 2 ), sf::Color( 0, 0, 0, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( 0.f, float( Lib::HEIGHT + _extraY ), float( Lib::WIDTH + _extraX * 2 ), float( Lib::HEIGHT + _extraY * 2 ), sf::Color( 0, 0, 0, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( 0.f, 0.f, float( _extraX - 2 ), float( Lib::HEIGHT + _extraY * 2 ), sf::Color( 32, 32, 32, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( 0.f, 0.f, float( Lib::WIDTH + _extraX * 2 ), float( _extraY - 2 ), sf::Color( 32, 32, 32, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( float( Lib::WIDTH + _extraX + 2 ), float( _extraY - 4 ), float( Lib::WIDTH + _extraX * 2 ), float( Lib::HEIGHT + _extraY * 2 ), sf::Color( 32, 32, 32, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( float( _extraX - 2 ), float( Lib::HEIGHT + _extraY + 2 ), float( Lib::WIDTH + _extraX * 2 ), float( Lib::HEIGHT + _extraY * 2 ), sf::Color( 32, 32, 32, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( float( _extraX - 4 ), float( _extraY - 4 ), float( _extraX - 2 ), float( Lib::HEIGHT + _extraY + 4 ), sf::Color( 128, 128, 128, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( float( _extraX - 4 ), float( _extraY - 4 ), float( Lib::WIDTH + _extraX + 4 ), float( _extraY - 2 ), sf::Color( 128, 128, 128, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( float( Lib::WIDTH + _extraX + 2 ), float( _extraY - 4 ), float( Lib::WIDTH + _extraX + 4 ), float( Lib::HEIGHT + _extraY + 4 ), sf::Color( 128, 128, 128, 255 ) ) );
    _window.Draw( sf::Shape::Rectangle( float( _extraX - 2 ), float( Lib::HEIGHT + _extraY + 2 ), float( Lib::WIDTH + _extraX + 4 ), float( Lib::HEIGHT + _extraY + 4 ), sf::Color( 128, 128, 128, 255 ) ) );
    _window.Display();
}
...
// Sounds
//------------------------------
#define USE_SOUND( sound, data ) sf::SoundBuffer* t_##sound = new sf::SoundBuffer(); t_##sound->LoadFromFile( data ); _sounds.push_back( SoundResource( 0, NamedSound( sound , t_##sound ) ) );
