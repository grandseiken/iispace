...
void HighScoreModal::render(Lib& lib) const
{
  ...
  if (is_high_score()) {
    ...
    lib.render_text(
        flvec2(4.f, 21.f), "It's a high score!", z0Game::PANEL_TEXT);
    lib.render_text(flvec2(4.f, 23.f),
                    players == 1 ? "Enter name:" : "Enter team name:",
                    z0Game::PANEL_TEXT);
    ...
    flvec2 low(float(4 * Lib::TEXT_WIDTH + 4),
               float(25 * Lib::TEXT_HEIGHT + 4));
    flvec2 hi(float(5 * Lib::TEXT_WIDTH - 4),
              float(26 * Lib::TEXT_HEIGHT - 4));
    lib.render_rect(low, hi, z0Game::PANEL_TEXT, 1);
  }
  ...
  if (_game.mode() == GameModal::BOSS_MODE) {
    ...
    lib.render_text(flvec2(4.f, b ? 6.f : 4.f),
                    "TIME ELAPSED: " + convert_to_time(score),
                    z0Game::PANEL_TEXT);
    std::stringstream ss;
  ...
  render_panel(lib, flvec2(3.f, 3.f),
...
void GameModal::render(Lib& lib) const
{
  ...
  if (_controllers_dialog) {
    render_panel(
        lib, flvec2(3.f, 3.f), flvec2(32.f, 8.f + 2 * players().size()));
...
GameModal::ship_list GameModal::ships_in_radius(
    const vec2& point, fixed radius, int32_t ship_mask) const
{
  ship_list r;
  for (auto& ship : _ships) {
    if ((!ship_mask || (ship->type() & ship_mask)) &&
...
GameModal::ship_list GameModal::collision_list(
    const vec2& point, int32_t category) const
{
  ship_list r;
  fixed x = point.x;
  fixed y = point.y;
...
bool z0Game::update()
{
  ...
  int32_t t = _selection;
  if (lib().is_key_pressed(Lib::KEY_UP)) {
    _selection = std::max(is_boss_mode_unlocked() ? -1 : 0, _selection - 1);
  }
  if (lib().is_key_pressed(Lib::KEY_DOWN)) {
    _selection = std::min(2, _selection + 1);
  }
  ...
  if (_selection == -1) {
    ...
    if (lib().is_key_pressed(Lib::KEY_LEFT)) {
      _special_selection = std::max(0, _special_selection - 1);
    }
    if (lib().is_key_pressed(Lib::KEY_RIGHT)) {
      _special_selection = std::min(
          is_what_mode_unlocked() ? 3 :
          is_fast_mode_unlocked() ? 2 :
          is_hard_mode_unlocked() ? 1 : 0, _special_selection + 1);
    }
  ...
  if (lib().is_key_pressed(Lib::KEY_ACCEPT) ||
    ...
    if (_selection == 0) {
      ...
      _modals.add(new GameModal(
          _lib, _save, _settings, &_frame_count, false,
          is_fast_mode_unlocked(), mode, _players));
    }
    if (_selection != 1) {
      lib().play_sound(Lib::SOUND_MENU_ACCEPT);
    }
...
void z0Game::render() const
{
  ...
  int32_t max = is_what_mode_unlocked() ? 3 :
                is_fast_mode_unlocked() ? 2 :
                is_hard_mode_unlocked() ? 1 : 0;
  if (is_boss_mode_unlocked()) {
    std::string str = _special_selection == 0 ? "BOSS MODE" :
                      _special_selection == 1 ? "HARD MODE" :
                      _special_selection == 2 ? "FAST MODE" : "W-HAT MODE";
  ...
  if (_selection == -1 && _special_selection < max) {
