...
const fixed Boss::HP_PER_EXTRA_PLAYER = M_PT_ONE;
const fixed Boss::HP_PER_EXTRA_CYCLE  = 3 * M_PT_ONE;

const int   BigSquareBoss::BASE_HP       = 400;
const fixed BigSquareBoss::SPEED         = M_TWO + M_HALF;
const int   BigSquareBoss::TIMER         = 100;
const int   BigSquareBoss::STIMER        = 80;
const fixed BigSquareBoss::ATTACK_RADIUS = M_ONE * 120;
...
const fixed DeathRayBoss::ARM_SPEED  = M_FOUR;
const int   DeathRayBoss::RAY_TIMER  = 100;
const int   DeathRayBoss::TIMER      = 100;
const fixed DeathRayBoss::SPEED      = M_FIVE;
...
// Generic boss
//------------------------------
Boss::Boss( const Vec2& position, z0Game::BossList boss, int hp, int players, int cycle, bool explodeOnDamage )
...
, _explodeOnDamage( explodeOnDamage )
{
    ...
    long s = 5000 * ( cycle + 1 ) + 2500 * ( boss > z0Game::BOSS_1C ) + 2500 * ( boss > z0Game::BOSS_2C );
    _score += s;
    for ( int i = 0; i < players - 1; ++i )
        _score += z_int( int( s ) * HP_PER_EXTRA_PLAYER );
...
int Boss::CalculateHP( int base, int players, int cycle )
{
    ...
    for ( int i = 0; i < cycle; ++i )
        r += z_int( hp * HP_PER_EXTRA_CYCLE );
    for ( int i = 0; i < players - 1; ++i )
        r += z_int( hp * HP_PER_EXTRA_PLAYER );
    for ( int i = 0; i < cycle * ( players - 1 ); ++i )
        r += z_int( hp * HP_PER_EXTRA_CYCLE + HP_PER_EXTRA_PLAYER );
    return r;
}
...
void Boss::Damage( int damage, bool magic, Player* source ) {
    ...
    actualDamage *= int( 60 / ( 1 + CountPlayers() - ( GetLives() == 0 ? Player::CountKilledPlayers() : 0 ) ) );
    _hp -= actualDamage;
...
void Boss::Render( bool hpBar ) const
{
    ...
    for ( unsigned int i = 0; i < CountShapes(); i++ ) {
        GetShape( i ).Render( GetLib(), Vec2f( GetPosition() ), z_float( GetRotation() ), int( i ) < _ignoreDamageColour ? 0xffffffff : 0 );
    }
    _damaged--;
}
...
void Boss::OnDestroy()
{
    ...
    for ( int i = 0; i < 16; ++i ) {
        Vec2 v;
        v.SetPolar( GetLib().RandFloat() * M_2PI, M_EIGHT + GetLib().RandInt( 64 ) + GetLib().RandInt( 64 ) );
        _fireworks.push_back( std::make_pair( n, std::make_pair( GetPosition() + v, GetShape( 0 ).GetColour() ) ) );
        n += i;
    }
...
: Boss( Vec2( Lib::WIDTH * M_PT_ZERO_ONE * 75, Lib::HEIGHT * M_TWO ), z0Game::BOSS_1A, BASE_HP, players, cycle )
, _dir( 0, -1 )
, _reverse( false )
, _timer( TIMER * 6 )
, _spawnTimer( 0 )
...
void BigSquareBoss::Update()
{
    ...
    if ( pos._y < Lib::HEIGHT * M_PT_ZERO_ONE * 25 && _dir._y == -1 )
        _dir.Set( _reverse ? 1 : -M_ONE, 0 );
    if ( pos._x < Lib::WIDTH * M_PT_ZERO_ONE * 25 && _dir._x == -1 )
        _dir.Set( 0, _reverse ? -M_ONE : M_ONE );
    if ( pos._y > Lib::HEIGHT * M_PT_ZERO_ONE * 75 && _dir._y == 1 )
        _dir.Set( _reverse ? -M_ONE : M_ONE, 0 );
    if ( pos._x > Lib::WIDTH * M_PT_ZERO_ONE * 75 && _dir._x == 1 )
        _dir.Set( 0, _reverse ? M_ONE : -M_ONE );
    ...
    if ( _specialAttack ) {
        ...
        if ( _attackPlayer->IsKilled() ) {
            ...
            for ( int i = 0; i < 6; i++ ) {
                ...
                s->SetRotation( M_PI / M_FOUR );
                s->SetScore( 0 );
                Spawn( s );
                d.Rotate( M_TWO * M_PI / M_SIX );
            }
            _attackPlayer = 0;
        ...
        _spawnTimer++;
        if ( _spawnTimer >= int( STIMER - ( CountPlayers() - Player::CountKilledPlayers() ) * 10 ) / ( IsHPLow() ? 2 : 1 ) ) {
            _spawnTimer = 0;
            _specialTimer++;
    ...
    GetShape( 1  ).Rotate( -M_PT_ZERO_ONE * 10 );
    GetShape( 2  ).Rotate(  M_PT_ZERO_ONE * 15 );
    GetShape( 3  ).Rotate( -M_PT_ZERO_ONE * 20 );
    GetShape( 4  ).Rotate(  M_PT_ZERO_ONE * 25 );
    GetShape( 5  ).Rotate( -M_PT_ZERO_ONE * 30 );
...
void BigSquareBoss::Render() const
{
    ...
    if ( ( _specialTimer / 4 ) % 2 && _attackPlayer ) {
        Vec2f d( z_float( ATTACK_RADIUS ), 0 );
        if ( _specialAttackRotate )
            d.Rotate( M_PIf / 2 );
        for ( int i = 0; i < 6; i++ ) {
            Vec2f p = Vec2f( _attackPlayer->GetPosition() ) + d;
            Polygon s( Vec2(), 10, 4, 0x9933ffff, M_PI / M_FOUR, 0 );
            s.Render( GetLib(), p, 0 );
            d.Rotate( 2 * M_PIf / 6 );
        }
    }
...
: Boss( Vec2( -Lib::WIDTH / M_TWO, Lib::HEIGHT / M_TWO ), z0Game::BOSS_1B, BASE_HP, players, cycle )
, _timer( 0 )
, _count( 0 )
, _unshielded( 0 )
, _attack( 0 )
...
, _shotAlternate( false )
{
    ...
    for ( int i = 0; i < 16; i++ ) {
        ...
        a.Rotate( i * M_PI / M_EIGHT );
        b.Rotate( i * M_PI / M_EIGHT );
...
{
    if ( !_side && GetPosition()._x < Lib::WIDTH * M_PT_ONE * 6 )
        Move( Vec2( 1, 0 ) * SPEED );
    else if ( _side && GetPosition()._x > Lib::WIDTH * M_PT_ONE * 4 )
        Move( Vec2( -1, 0 ) * SPEED );

    Rotate( -M_PT_ZERO_ONE * 2 );
    GetShape( 0 ).Rotate( M_PT_ZERO_ONE * 4 );
    ...
    if ( _attack ) {
        Vec2 d( _attackDir );
        d.Rotate( ( ATTACK_TIME - _attack ) * M_HALF * M_PI / ATTACK_TIME );
    ...
    if ( _timer > TIMER ) {
        ...
        if ( _count >= 4 && ( !GetLib().RandInt( 4 ) || _count >= 8 ) ) {
            _count = 0;
            if ( !_unshielded ) {
                z0Game::ShipList list = GetShipsInRadius( GetPosition(), Lib::WIDTH * 2 );
                int p = 0;
        ...
        if ( GetLib().RandInt( 2 ) ) {
            ...
            for ( int i = 0; i < 12; i++ ) {
                Spawn( new SBBossShot( GetPosition(), d ) );
                d.Rotate( M_TWO * M_PI / ( 2 * M_SIX ) );
            }
            PlaySound( Lib::SOUND_BOSS_ATTACK );
        }
        else {
            _attack = ATTACK_TIME;
            _attackDir.SetPolar( GetLib().RandFloat() * M_2PI, 5 );
        }
    }
}
...
int ShieldBombBoss::GetDamage( int damage, bool magic )
{
    ...
    if ( _shotAlternate )
        RestoreHP( int( 60 / ( 1 + CountPlayers() - ( GetLives() == 0 ? Player::CountKilledPlayers() : 0 ) ) ) );
    return damage;
}
...
static const fixed HP_REDUCE_POWER_lookup[ 8 ] = {
    ...
    M_FOUR + 9 * M_PT_ONE,
    M_EIGHT + 4 * M_PT_ONE,
    M_TEN + M_FOUR + 2 * M_PT_ONE,
    2 * M_TEN + M_FOUR + 2 * M_PT_ONE,
    4 * M_TEN + M_ONE
};
...
static const fixed ONE_AND_HALF_lookup[ 8 ] = {
    ...
    M_FIVE + M_PT_ONE,
    M_SEVEN + 6 * M_PT_ONE,
    M_TEN + M_ONE + 4 * M_PT_ONE,
    M_TEN + M_SEVEN + M_PT_ONE
};
...
static const int ONE_PT_ONE_FIVE_intLookup[ 128 ] = {
    1, 1, 1, 2, 2, 2, 2, 3, 3, 4, 4, 5, 5, 6, 7, 8, 9,
    11, 12, 14, 16, 19, 22, 25, 29, 33, 38, 44, 50, 58, 66, 76, 88,
    101, 116, 133, 153, 176, 203, 233, 268, 308, 354, 407, 468, 539, 620, 713, 819,
    942, 1084, 1246, 1433, 1648, 1895, 2180, 2507, 2883, 3315, 3812, 4384, 5042, 5798, 6668, 7668,
    8818, 10140, 11662, 13411, 15422, 17736, 20396, 23455, 26974, 31020, 35673, 41024, 47177, 54254, 62392, 71751,
    82513, 94890, 109124, 125493, 144316, 165964, 190858, 219487, 252410, 290272, 333813, 383884, 441467, 507687, 583840, 671416,
    772129, 887948, 1021140, 1174311, 1350458, 1553026, 1785980, 2053877, 2361959, 2716252, 3123690, 3592244, 4131080, 4750742, 5463353, 6282856,
    7225284, 8309077, 9555438, 10988754, 12637066, 14532626, 16712520, 19219397, 22102306, 25417652, 29230299, 33614843, 38657069, 44455628, 51123971
};

ChaserBoss::ChaserBoss( int players, int cycle, int split, const Vec2& position, int time, int stagger )
: Boss( !split ? Vec2( Lib::WIDTH / M_TWO, -Lib::HEIGHT / M_TWO ) : position, z0Game::BOSS_1C, 1 + BASE_HP / z_int( M_HALF + HP_REDUCE_POWER_lookup[ split ] ), players, cycle, split <= 1 )
, _onScreen( split != 0 )
, _move( false )
, _timer( time )
, _dir()
...
, _stagger( stagger )
{
    AddShape( new Polygram( Vec2(),  10 * ONE_AND_HALF_lookup[ MAX_SPLIT - _split ], 5, 0x3399ffff, 0, 0 ) );
    AddShape( new Polygram( Vec2(),   9 * ONE_AND_HALF_lookup[ MAX_SPLIT - _split ], 5, 0x3399ffff, 0, 0 ) );
    AddShape( new Polygram( Vec2(),   8 * ONE_AND_HALF_lookup[ MAX_SPLIT - _split ], 5, 0, 0, ENEMY | VULNERABLE ) );
    AddShape( new Polygram( Vec2(),   7 * ONE_AND_HALF_lookup[ MAX_SPLIT - _split ], 5, 0, 0, SHIELD ) );
...
{
    z0Game::ShipList remaining = GetShipsInRadius( Vec2( Lib::WIDTH / M_TWO, Lib::HEIGHT / M_TWO ), Lib::WIDTH * Lib::HEIGHT );
    _lastDir = _dir;
    _lastDir.Normalise();
    if ( IsOnScreen() )
        _onScreen = true;
    ...
    if ( _timer <= 0 ) {
        ...
        if ( _split != 0 && ( _move || GetLib().RandInt( 8 + _split ) == 0 || int( remaining.size() ) - CountPlayers() <= 4 ||
            GetLib().RandInt( ONE_PT_ONE_FIVE_intLookup[ std::max( 0, std::min( 127, int( remaining.size() ) - CountPlayers() ) ) ] ) == 0 ) )
            _move = !_move;
        if ( _move ) {
    ...
    if ( _move ) {
        ...
        static const fixed c_attract = -( 1 + 2 * M_PT_ONE );
        ...
        static const fixed c_rotate = ( M_PT_ZERO_ONE * 5 ) / fixed( MAX_SPLIT );

        _dir = _lastDir;
        if ( _stagger == _count % ( _split == 0 ? 1 : _split == 1 ? 2 : _split == 2 ? 4 : _split == 3 ? 8 : 16 ) ) {
            _dir._x = _dir._y = 0;
            for ( std::size_t i = 0; i < nearby.size(); ++i ) {
                if ( nearby[ i ] == this || !( nearby[ i ]->IsBoss() || nearby[ i ]->IsPlayer() ) )
                    continue;
                ...
                if ( nearby[ i ]->IsBoss() ) {
                    ...
                    p.SetPolar( ( ( Player* )nearby[ i ] )->GetRotation(), M_ONE );
        ...
        if ( int( remaining.size() ) - CountPlayers() < 4 && _split >= MAX_SPLIT - 1 ) {
            if ( ( GetPosition()._x < 32 && _dir._x < 0 ) || ( GetPosition()._x >= Lib::WIDTH - 32 && _dir._x > 0 ) )
                _dir._x = -_dir._x;
            if ( ( GetPosition()._y < 32 && _dir._y < 0 ) || ( GetPosition()._y >= Lib::HEIGHT - 32 && _dir._y > 0 ) )
                _dir._y = -_dir._y;
        }
        else if ( int( remaining.size() ) - CountPlayers() < 8 && _split >= MAX_SPLIT - 1 ) {
            if ( ( GetPosition()._x < 0 && _dir._x < 0 ) || ( GetPosition()._x >= Lib::WIDTH && _dir._x > 0 ) )
                _dir._x = -_dir._x;
            if ( ( GetPosition()._y < 0 && _dir._y < 0 ) || ( GetPosition()._y >= Lib::HEIGHT && _dir._y > 0 ) )
                _dir._y = -_dir._y;
        }
        else {
            if ( ( GetPosition()._x < -32 && _dir._x < 0 ) || ( GetPosition()._x >= Lib::WIDTH + 32 && _dir._x > 0 ) )
        ...
        if ( GetPosition()._x < -256 )
            ...
            _dir.Normalise();
        _dir = _split == 0 ? _dir + _lastDir * 7 : _split == 1 ? _dir * 2 + _lastDir * 7 : _split == 2 ? _dir * 4 + _lastDir * 7 : _split == 3 ? _dir + _lastDir : _dir * 2 + _lastDir;
        _dir *= ONE_PT_ONE_lookup[ _split ] * ( _split == 0 ? c_dir0 : _split == 1 ? c_dir1 : _split == 2 ? c_dir2 : _split == 3 ? c_dir3 : c_dir4 );
        Move( _dir );
        Rotate( ( M_PT_ZERO_ONE * 2 ) + fixed( _split ) * c_rotate );
    }
    _sharedHp = 0;
    if ( !_hasCounted ) {
        _hasCounted = true;
...
void ChaserBoss::Render() const
{
    ...
    if ( hpLookup.empty() ) {
        int hp = 0;
        for ( int i = 0; i < 8; i++ ) {
            hp = 2 * hp + CalculateHP( 1 + BASE_HP / z_int( M_HALF + HP_REDUCE_POWER_lookup[ 7 - i ] ), _players, _cycle );
            hpLookup.push_back( hp );
        }
    }
    _sharedHp += ( _split == 7 ? 0 : 2 * hpLookup[ 6 - _split ] ) + GetRemainingHP() * 30;
    if ( _onScreen )
...
void ChaserBoss::OnDestroy()
{
    ...
    if ( _split < MAX_SPLIT ) {
        for ( int i = 0; i < 2; i++ ) {
            Vec2 d;
            d.SetPolar( i * M_PI + GetRotation(), 8 * ONE_PT_TWO_lookup[ MAX_SPLIT - 1 - _split ] );
            Ship* s = new ChaserBoss( _players, _cycle, _split + 1, GetPosition() + d, ( i + 1 ) * TIMER / 2, GetLib().RandInt( _split + 1 == 1 ? 2 : _split + 1 == 2 ? 4 : _split + 1 == 3 ? 8 : 16 ) );
            Spawn( s );
...
: Boss( Vec2( Lib::WIDTH * ( M_ONE + M_HALF ), Lib::HEIGHT / M_TWO ), z0Game::BOSS_2A, BASE_HP, players, cycle )
, _willAttack( false )
, _stopped( false )
, _generating( false )
, _attacking( false )
...
{
    if ( GetPosition()._x <= Lib::WIDTH / M_TWO && _willAttack && !_stopped && !_continue ) {
        _stopped = true;
        _generating = true;
    ...
    if ( GetPosition()._x < -150 ) {
        ...
        _continue = false;
        _sound = !_willAttack;
        Rotate( GetLib().RandFloat() * 2 * M_PI );
    }
    ...
    if ( !_stopped ) {
        ...
        if ( !_willAttack && _timer % ( 16 - ( CountPlayers() - Player::CountKilledPlayers() ) * 2 ) == 0 && IsOnScreen() ) {
            if ( _shootType == 0 || ( IsHPLow() && _shootType == 1 ) ) {
                ...
                d.Normalise();
                Spawn( new SBBossShot( v, d * M_FIVE, 0xcc33ccff ) );
                Spawn( new SBBossShot( v, d * -M_FIVE, 0xcc33ccff ) );
        ...
        if ( _generating ) {
            ...
            if ( _timer % ( 10 - 2 * ( CountPlayers() - Player::CountKilledPlayers() ) ) == 0 && _timer < TIMER * 4 ) {
                Ship* s = new TBossShot( _s1->ConvertPoint( GetPosition(), GetRotation(), Vec2() ), _genDir ? GetRotation() + M_PI : GetRotation() );
                Spawn( s );
                s = new TBossShot( _s2->ConvertPoint( GetPosition(), GetRotation(), Vec2() ), _genDir ? GetRotation() : GetRotation() + M_PI );
                Spawn( s );
                PlaySoundRandom( Lib::SOUND_ENEMY_SPAWN );
            }

            if ( IsHPLow() && _timer % ( 20 - ( CountPlayers() - Player::CountKilledPlayers() ) * 2 ) == 0 ) { 
                Player* p = GetNearestPlayer();
                Vec2 v = GetPosition();
            ...
            if ( !_attacking ) {
                ...
                if ( _timer % z_int( TIMER / ( M_ONE + M_HALF ) ) == TIMER / 8 ) {
                    Vec2 v = _s1->ConvertPoint( GetPosition(), GetRotation(), Vec2() );
                    Vec2 d;
                    d.SetPolar( GetLib().RandFloat() * M_2PI, M_FIVE );
                ...
                for ( unsigned int i = 0; i < t.size(); i++ ) {
                    ...
                    d.Normalise();
                    t[ i ]->Move( d * speed );
                    if ( t[ i ]->IsEnemy() && !t[ i ]->IsWall() && ( t[ i ]->GetPosition() - GetPosition() ).Length() <= 40 ) {
                        t[ i ]->Destroy();
                        _attackSize++;
                        _sAttack->SetRadius( _attackSize / ( M_ONE + M_HALF ) );
                ...
                for ( int i = 0; i < _attackSize; i++ ) {
                    Vec2 d;
                    d.SetPolar( i * M_2PI / _attackSize, M_FIVE );
    ...
    for ( unsigned int i = _attackShapes; i < CountShapes(); ++i ) {
        Vec2 v;
        v.SetPolar( GetLib().RandFloat() * M_2PI, 2 * ( GetLib().RandFloat() - M_HALF ) * fixed( _attackSize ) / ( M_ONE + M_HALF ) );
    ...
    _s1->Rotate(  M_PT_ONE / 2 );
    _s2->Rotate( -M_PT_ONE / 2 );
    for ( int i = 0; i < 8; i++ ) {
        _s1->GetShape( i + 4 ).Rotate( -M_PT_ONE );
        _s2->GetShape( i + 4 ).Rotate(  M_PT_ONE );
    }
}
...
void TractorBoss::Render() const
{
    ...
    if ( ( _stopped && !_generating && !_attacking ) ||
        ...
        for ( std::size_t i = 0; i < _targets.size(); ++i ) {
            if ( ( ( _timer + i * 4 ) / 4 ) % 2 ) {
                GetLib().RenderLine( Vec2f( GetPosition() ), Vec2f( _targets[ i ] ), 0xcc33ccff );
            }
        }
    }
}
...
: Boss( Vec2( Lib::WIDTH / M_TWO, Lib::HEIGHT / M_TWO ), z0Game::BOSS_2B, BASE_HP, players, cycle )
, _visible( false )
, _vTime( 0 )
, _timer( 0 )
...
, _shotType( false )
{
    ...
    for ( int i = 0; i < 8; i++ ) {
        Vec2 c;
        c.SetPolar( i * M_PI / M_FOUR, 48 );

        CompoundShape* s = new CompoundShape( c, 0, 0 );
        for ( int j = 0; j < 8; j++ ) {
            Vec2 d;
            d.SetPolar( j * M_PI / M_FOUR, M_ONE );
            s->AddShape( new Line( Vec2(), d * M_TEN, d * M_TEN * 2, 0x9933ccff, 0 ) );
        }
        AddShape( s );
    }
    ...
    for ( int n = 0; n < 5; n++ ) {
        ...
        for ( int i = 0; i < 16 + n * 6; i++ ) {
            Vec2 d;
            d.SetPolar( i * M_TWO * M_PI / ( 16 + n * 6 ), fixed( 100 + n * 60 ) );
...
void GhostBoss::Update()
{
    for ( int n = 1; n < 5; ++n ) {
        ...
        for ( int i = 0; i < 16 + n * 6; ++i ) {
            ...
            int t = n == 1 ? ( i + _dangerOffset1 ) % 11 :
                    n == 2 ? ( i + _dangerOffset2 ) %  7 :
                    n == 3 ? ( i + _dangerOffset3 ) % 17 :
                             ( i + _dangerOffset4 ) % 10 ;
            bool b = n == 1 ? ( t % 11 == 0 || t % 11 == 5 ) :
                     n == 2 ?                ( t %  7 == 0 ) :
                     n == 3 ? ( t % 17 == 0 || t % 17 == 8 ) :
                                             ( t % 10 == 0 ) ;
            if ( ( ( n == 4 && _attack != 2 ) || ( n + ( n == 3 ) ) & _dangerCircle ) && _visible && b ) {
    ...
    if ( !( _attack == 2 && _attackTime < ATTACK_TIME * 4 && _attackTime ) )
        Rotate( -M_PT_ZERO_ONE * 4 );

    for ( int i = 0; i < 8; i++ ) {
        GetShape( i + 2 ).Rotate( M_PT_ZERO_ONE * 4 );
    }
    GetShape( 10 ).Rotate( M_PT_ZERO_ONE * 6 );
    for ( int n = 0; n < 5; n++ ) {
        CompoundShape* s = ( CompoundShape* )&GetShape( 11 + n );
        if ( n % 2 )
            s->Rotate( M_PT_ZERO_ONE * 3 + ( M_THREE / 2000 ) * n );
        else
            s->Rotate( M_PT_ZERO_ONE * 5 - ( M_THREE / 2000 ) * n );
        for ( std::size_t i = 0; i < s->CountShapes(); i++ )
            s->GetShape( i ).Rotate( -M_PT_ONE );
        s = ( CompoundShape* )&GetShape( 16 + n );
        if ( n % 2 )
            s->Rotate( M_PT_ZERO_ONE * 3 + ( M_THREE / 2000 ) * n );
        else
            s->Rotate( M_PT_ZERO_ONE * 4 - ( M_THREE / 2000 ) * n );
        for ( std::size_t i = 0; i < s->CountShapes(); i++ )
            s->GetShape( i ).Rotate( -M_PT_ONE );
    }
    ...
    if ( !_attackTime ) {
        _timer++;
        if ( _timer == 9 * TIMER / 10 || ( _timer >= TIMER / 10 && _timer < 9 * TIMER / 10 - 16 && ( ( _timer % 16 == 0 && _attack == 2 ) || ( _timer % 32 == 0 && _shotType ) ) ) ) {
            for ( int i = 0; i < 8; i++ ) {
                Vec2 d;
                d.SetPolar( i * M_PI / M_FOUR + GetRotation(), M_FIVE );
                Spawn( new SBBossShot( GetPosition(), d, 0xcc66ffff ) );
            }
            PlaySoundRandom( Lib::SOUND_BOSS_FIRE );
        }
        if ( _timer != 9 * TIMER / 10 && _timer >= TIMER / 10 && _timer < 9 * TIMER / 10 - 16 && _timer % 16 == 0 && ( !_shotType || _attack == 2 ) ) {
            Player* p = GetNearestPlayer();
            Vec2 d = p->GetPosition() - GetPosition();
            d.Normalise();

            if ( d.Length() > M_HALF ) {
                Spawn( new SBBossShot( GetPosition(), d *    M_FIVE,   0xcc66ffff ) );
                Spawn( new SBBossShot( GetPosition(), d * ( -M_FIVE ), 0xcc66ffff ) );
                PlaySoundRandom( Lib::SOUND_BOSS_FIRE );
            }
        }
        ...
        if ( _attack == 2 ) {
            if ( _attackTime >= ATTACK_TIME * 4 && _attackTime % 8 == 0 ) {
                Vec2 pos( fixed( GetLib().RandInt( Lib::WIDTH + 1 ) ), fixed( GetLib().RandInt( Lib::HEIGHT + 1 ) ) );
                ...
                _visible = true;
                _vTime = 60;
                AddShape( new Box( Vec2( Lib::WIDTH / M_TWO + M_ONE * 32, 0 ), Lib::WIDTH / M_TWO, M_TEN, 0xcc66ffff, 0, 0 ) );
                AddShape( new Box( Vec2( Lib::WIDTH / M_TWO + M_ONE * 32, 0 ), Lib::WIDTH / M_TWO, M_SEVEN,  0xcc66ffff, 0, 0 ) );
                AddShape( new Box( Vec2( Lib::WIDTH / M_TWO + M_ONE * 32, 0 ), Lib::WIDTH / M_TWO, M_FOUR,  0xcc66ffff, 0, 0 ) );
                ...
                if ( _attackTime == ATTACK_TIME * 3 - 1 )
                    PlaySound( Lib::SOUND_BOSS_ATTACK );
                Rotate( ( _rDir ? 1 : -1 ) * M_TWO * M_PI / ( ATTACK_TIME * M_SIX ) );
                if ( !_attackTime ) {
                    for ( int i = 0; i < 6; i++ )
                        DestroyShape( 21 );
                }
        ...
        if ( ( _attack == 0 || _attack == 1 ) && IsHPLow() && _attackTime == ATTACK_TIME * 1 ) {
            Vec2 v;
...
: Boss( Vec2( Lib::WIDTH * ( M_THREE / 20 ), -Lib::HEIGHT ), z0Game::BOSS_2C, BASE_HP, players, cycle )
, _timer( TIMER * 2 )
, _laser( false )
, _dir( true )
, _pos( 1 )
...
, _raySrc2()
{
    AddShape( new Polystar( Vec2(), 110, 12, 0x228855ff, M_PI / ( 2 * M_SIX ), 0 ) );
    AddShape( new Polygram( Vec2(), 70,  12, 0x33ff99ff, M_PI / ( 2 * M_SIX ), 0 ) );
    AddShape( new Polygon ( Vec2(), 120, 12, 0x33ff99ff, M_PI / ( 2 * M_SIX ), ENEMY | VULNERABLE ) );
    AddShape( new Polygon ( Vec2(), 115, 12, 0x33ff99ff, M_PI / ( 2 * M_SIX ), 0 ) );
    AddShape( new Polygon ( Vec2(), 110, 12, 0x33ff99ff, M_PI / ( 2 * M_SIX ), SHIELD ) );

    CompoundShape* s1 = new CompoundShape( Vec2(), 0, ENEMY );
    for ( int i = 1; i < 12; i++ ) {
        CompoundShape* s2 = new CompoundShape( Vec2(), i * M_PI / M_SIX, 0 );
    ...
    Rotate( M_2PI * fixed( z_rand() ) / Z_RAND_MAX );
}

void DeathRayBoss::Update()
{
    bool positioned = true;
    fixed d = _pos == 0 ? M_ONE   * Lib::HEIGHT / M_FOUR  - GetPosition()._y :
              _pos == 1 ? M_TWO   * Lib::HEIGHT / M_FOUR  - GetPosition()._y :
              _pos == 2 ? M_THREE * Lib::HEIGHT / M_FOUR  - GetPosition()._y :
              _pos == 3 ? M_ONE   * Lib::HEIGHT / M_EIGHT - GetPosition()._y :
              _pos == 4 ? M_THREE * Lib::HEIGHT / M_EIGHT - GetPosition()._y :
              _pos == 5 ? M_FIVE  * Lib::HEIGHT / M_EIGHT - GetPosition()._y :
                          M_SEVEN * Lib::HEIGHT / M_EIGHT - GetPosition()._y ;
    if ( z_abs( d ) > 3 ) {
        Move( Vec2( 0, d / z_abs( d ) ) * SPEED );
        positioned = false;
    }
    ...
    if ( _rayAttackTimer ) {
        ...
        if ( _rayAttackTimer < 40 ) {
            ...
            d.Normalise();
            Spawn( new SBBossShot( GetPosition(), d * M_TEN, 0xccccccff ) );
    ...
    if ( _laser ) {
        ...
        if ( _timer ) {
            ...
            if ( r == 0 )
                _timer = TIMER * 2;
            if ( r < M_PT_ONE || r > M_TWO * M_PI - M_PT_ONE )
                SetRotation( 0 );
            else {
                goingFast = true;
                Rotate( _dir ? M_PT_ONE : -M_PT_ONE );
            }
        }
    }
    else {
        Rotate( _dir ? M_PT_ZERO_ONE * 2 : -M_PT_ZERO_ONE * 2 );
        if ( IsOnScreen() ) {
            _timer--;
    ...
    if ( _arms.empty() && IsHPLow() ) {
        ...
        if ( _shotTimer % 128 == 0 ) {
            _rayAttackTimer = RAY_TIMER;
            Vec2 d1, d2;
            d1.SetPolar( GetLib().RandFloat() * M_2PI, M_ONE * 110 );
            d2.SetPolar( GetLib().RandFloat() * M_2PI, M_ONE * 110 );
    ...
    if ( _arms.empty() ) {
        ...
        if ( _armTimer >= ARM_RTIMER ) {
            _armTimer = 0;
            if ( !IsHPLow() ) {
                int hp = z_int( ARM_HP * ( 7 * M_PT_ONE + 3 * M_PT_ONE * int32_t( GetLives() ? CountPlayers() : CountPlayers() - Player::CountKilledPlayers() ) ) );
    ...
    for ( std::size_t i = 0; i < _shotQueue.size(); ++i ) {
        if ( !goingFast || _shotTimer % 2 ) {
            ...
            Ship* s = new SBBossShot( GetPosition() + d * 120, d * M_FIVE, 0x33ff99ff );
            Spawn( s );
        }
        _shotQueue[ i ].second--;
...
// Super boss
//------------------------------
SuperBossArc::SuperBossArc( const Vec2& position, int players, int cycle, int i, Ship* boss, int timer )
...
, _sTimer( 0 )
{
    AddShape( new PolyArc( Vec2(), 140, 32, 2, 0, i * M_TWO * M_PI / 16, 0 ) );
    AddShape( new PolyArc( Vec2(), 135, 32, 2, 0, i * M_TWO * M_PI / 16, 0 ) );
    AddShape( new PolyArc( Vec2(), 130, 32, 2, 0, i * M_TWO * M_PI / 16, 0 ) );
    AddShape( new PolyArc( Vec2(), 125, 32, 2, 0, i * M_TWO * M_PI / 16, SHIELD ) );
    AddShape( new PolyArc( Vec2(), 120, 32, 2, 0, i * M_TWO * M_PI / 16, 0 ) );
    AddShape( new PolyArc( Vec2(), 115, 32, 2, 0, i * M_TWO * M_PI / 16, 0 ) );
    AddShape( new PolyArc( Vec2(), 110, 32, 2, 0, i * M_TWO * M_PI / 16, 0 ) );
    AddShape( new PolyArc( Vec2(), 105, 32, 2, 0, i * M_TWO * M_PI / 16, SHIELD ) );
}
...
void SuperBossArc::Update()
{
    Rotate( M_SIX / 1000 );
    for ( int i = 0; i < 8; ++i )
        GetShape( 7 - i ).SetColour( GetLib().Cycle( IsHPLow() ? 0xff000099 : 0xff0000ff, ( i * 32 + 2 * _timer ) % 256 ) );
    ++_timer;
    ++_sTimer;
    if ( _sTimer == 64 )
        GetShape( 0 ).SetCategory( ENEMY | VULNERABLE );
...
{
    Vec2 d;
    d.SetPolar( _i * M_TWO * M_PI / 16 + GetRotation(), 120 );
    Move( d );
    Explosion();
    Explosion( 0xffffffff, 12 );
    Explosion( GetShape( 0 ).GetColour(), 24 );
...
: Boss( Vec2( Lib::WIDTH * M_HALF, -Lib::HEIGHT / ( M_TWO + M_HALF ) ), z0Game::BOSS_3A, BASE_HP, players, cycle )
, _players( players )
, _cycle( cycle )
, _cTimer( 0 )
, _timer( 0 )
...
void SuperBoss::Update()
{
    if ( _arcs.empty() ) {
        for ( int i = 0; i < 16; ++i ) {
            SuperBossArc* s = new SuperBossArc( GetPosition(), _players, _cycle, i, this );
            Spawn( s );
            _arcs.push_back( s );
        }
    }
    ...
    for ( int i = 0; i < 8; ++i )
        GetShape( 7 - i ).SetColour( GetLib().Cycle( 0xff0000ff, ( i * 32 + 2 * _cTimer ) % 256 ) );
    ++_cTimer;
    ...
    static const fixed d5d1000 = fixed( M_FIVE / 1000 );
    static const fixed pi2d64 = M_2PI / 64;
    static const fixed pi2d32 = M_2PI / 32;
    if ( _state == STATE_IDLE && IsOnScreen() && _timer != 0 && _timer % 300 == 0 ) {
        ...
        if ( r == 0 || r == 1 ) {
            ...
            _state = STATE_ATTACK;
            _timer = 0;
            fixed f = GetLib().RandFloat() * M_2PI;
    ...
    if ( _state == STATE_IDLE && IsOnScreen() && _timer % 300 == 200 && !IsDestroyed() ) {
        ...
        if ( !wide3.empty() ) {
            ...
            SuperBossArc* s = new SuperBossArc( GetPosition(), _players, _cycle, wide3[ r ], this, timer );
            s->SetRotation( GetRotation() - ( M_SIX / 1000 ) );
            Spawn( s );
            _arcs[ wide3[ r ] ] = s;
    ...
    static const fixed pi2d128 = M_2PI / 128;
    ...
    if ( _snakes ) {
        --_snakes;
        Vec2 d;
        d.SetPolar( GetLib().RandFloat() * M_2PI, fixed( GetLib().RandInt( 32 ) ) + GetLib().RandInt( 16 ) );
...
void SuperBoss::OnDestroy()
{
    ...
    for ( int i = 0; i < 16; ++i ) {
        ...
        _fireworks.push_back( std::make_pair( n, std::make_pair( GetPosition() + v, c ) ) );
        n += i;
    }
    for ( int i = 0; i < Lib::PLAYERS; i++ ) {
        GetLib().Rumble( i, 25 );
