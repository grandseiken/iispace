...
Fix32 Fix32::sqrt() const
{
  ...
  for (uint8_t n = 0; n < 8 && f != 0; ++n) {
    f = f * half + a / f;
    if (from_internal(fixed_abs((f * f)._value - _value)) < bound) {
      break;
    }
  }
  return f;
...
Fix32 Fix32::sin() const
{
  ...
  Fix32 angle = from_internal(
      (_value < 0 ? -_value : _value) % (F32PI * 2)._value);
...
Fix32 Fix32::atan2(const Fix32& x) const
{
  ...
  if (x._value >= 0) {
    ...
    Fix32 r = (x - y) / (x + y);
    Fix32 r3 = r * r * r;
    angle = from_internal(0x32400000) * r3 -
            from_internal(0xfb500000) * r + PId4;
  }
  else {
    if (x - y == 0) {
      return -PI3d4;
    ...
    Fix32 r3 = r * r * r;
    angle = from_internal(0x32400000) * r3 -
            from_internal(0xfb500000) * r + PI3d4;
  }
