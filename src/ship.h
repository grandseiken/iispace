...
public:
    ...
    //------------------------------
    Ship( const Vec2& position, bool particle = false, bool player = false, bool boss = false );
    virtual ~Ship();
    ...
    void Explosion( Colour c = 0, int time = 8, bool towards = false, const Vec2f& v = Vec2f() ) const;
    ...
    {
        return GetLib().PlaySound( sound, 1.f, 2.f * z_float( GetPosition()._x ) / Lib::WIDTH - 1.f );
    }
    bool PlaySoundRandom( Lib::Sound sound, float pitch = 0.f, float volume = 1.f )
    {
        return GetLib().PlaySound( sound, volume * ( .5f * z_float( GetLib().RandFloat() ) + .5f ), 2.f * z_float( GetPosition()._x ) / Lib::WIDTH - 1.f, pitch );
    }
...
public:
    ...
    Particle( const Vec2f& position, Colour colour, const Vec2f& velocity, int time )
    : _destroy( false ), _position( position ), _velocity( velocity ), _timer( time ), _colour( colour ), _z0( 0 ) { }
