...
public:
    ...
    virtual Vec2 ConvertPoint( const Vec2& position, fixed rotation, const Vec2& v ) const
    {
        Vec2 a = GetCentre() + v;
        a.Rotate( rotation );
        return position + a;
    ...
    virtual Vec2f ConvertPointf( const Vec2f& position, float rotation, const Vec2f& v ) const
    {
    ...
    virtual void Render( Lib& lib, const Vec2f& position, float rotation, Colour colour = 0 ) const = 0;

    virtual fixed GetRotation() const { return 0; }
...
public:
    ...
    virtual void SetRotation( fixed rotation )
    {
        _rotation = ( ( rotation > M_2PI ) ? ( rotation - M_2PI ) : ( rotation < 0 ? rotation + M_2PI : rotation ) );
    }
...
public:
    ...
    Fill( const Vec2& centre, fixed width, fixed height, Colour colour, int category = 0 )
    ...
    virtual void Render( Lib& lib, const Vec2f& position, float rotation, Colour colour = 0 ) const
    {
...
public:
    ...
    Line( const Vec2& centre, const Vec2& a, const Vec2& b, Colour colour, fixed rotation = 0 )
...
public:
    ...
    Box( const Vec2& centre, fixed width, fixed height, Colour colour, fixed rotation = 0, int category = 0 )
...
public:
    ...
    Polygon( const Vec2& centre, fixed radius, int sides, Colour colour, fixed rotation = 0, int category = 0 )
    ...
    virtual void Render( Lib& lib, const Vec2f& position, float rotation, Colour colour = 0 ) const
    {
        ...
        for ( int i = 0; i < GetSides(); i++ ) {
            ...
            lib.RenderLine( ConvertPointf( position, rotation, a ), ConvertPointf( position, rotation, b ), GetColour( colour ) );
        }
    }
...
public:
    ...
    PolyArc( const Vec2& centre, fixed radius, int sides, int segments, Colour colour, fixed rotation = 0, int category = 0 )
    : RotateShape( centre, rotation, colour, category ), _radius( radius ), _sides( sides ), _segments( segments ) { }
    virtual ~PolyArc() { }
...
public:
    ...
    Polygram( const Vec2& centre, fixed radius, int sides, Colour colour, fixed rotation = 0, int category = 0 )
    ...
    virtual void Render( Lib& lib, const Vec2f& position, float rotation, Colour colour = 0 ) const
    {
        ...
        for ( unsigned int i = 0; i < list.size(); i++ ) {
            for ( unsigned int j = i + 1; j < list.size(); j++ ) {
                lib.RenderLine( ConvertPointf( position, rotation, list[ i ] ), ConvertPointf( position, rotation, list[ j ] ), GetColour( colour ) );
            }
        }
    }
...
public:
    ...
    Polystar( const Vec2& centre, fixed radius, int sides, Colour colour, fixed rotation = 0, int category = 0 )
    : Polygon( centre, radius, sides, colour, rotation, category ) { }
    virtual ~Polystar() { }
    ...
    virtual void Render( Lib& lib, const Vec2f& position, float rotation, Colour colour = 0 ) const
    {
        ...
        for ( int i = 0; i < GetSides(); i++ ) {
            Vec2f v;
            v.SetPolar( i * 2 * M_PIf / float( GetSides() ), r );
            lib.RenderLine( ConvertPointf( position, rotation, Vec2f() ), ConvertPointf( position, rotation, v ), GetColour( colour ) );
        }
    }
...
public:
    ...
    CompoundShape( const Vec2& centre, fixed rotation = 0, int category = 0, Colour colour = 0 )
    : RotateShape( centre, rotation, 0, category ) { SetColour( colour ); }
