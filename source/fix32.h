...
#include <cmath>
#include <ostream>
inline int64_t fixedabs( int64_t f )
{
    return ( f ^ ( f >> 63 ) ) - ( f >> 63 );
}

inline int64_t fixedsgn( int64_t a, int64_t b )
{
    return ( ( ( a < 0 ) == ( b < 0 ) ) << 1 ) - 1;
}
// MATT BELL CODE
...
public:
    ...
    inline Fix32 operator*( const Fix32& f ) const
    {
        int64_t sign = fixedsgn( _value, f._value );
        uint64_t l = fixedabs( _value );
        uint64_t r = fixedabs( f._value );
    ...
    {
        int64_t sign = fixedsgn( _value, f._value );
        uint64_t r = fixedabs( _value );
        uint64_t d = fixedabs( f._value );
        uint64_t q = 0;
