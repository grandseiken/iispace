...
void SBBossShot::Update()
{
    Move( _dir );
    Vec2 p = GetPosition();
    if ( ( p._x < -10 && _dir._x < 0 ) || ( p._x > Lib::WIDTH + 10 && _dir._x > 0 ) || ( p._y < -10 && _dir._y < 0 ) || ( p._y > Lib::HEIGHT + 10 && _dir._y > 0 ) )
        Destroy();
...
{
    if ( noGap ) {
        AddShape( new Box( Vec2(), fixed( Lib::WIDTH ), M_TEN, 0xcc66ffff, 0, ENEMY | SHIELD ) );
        AddShape( new Box( Vec2(), fixed( Lib::WIDTH ), M_SEVEN, 0xcc66ffff, 0, 0 ) );
        AddShape( new Box( Vec2(), fixed( Lib::WIDTH ), M_FOUR, 0xcc66ffff, 0, 0 ) );
    } else {
        AddShape( new Box( Vec2(), Lib::HEIGHT / 2, M_TEN, 0xcc66ffff, 0, ENEMY | SHIELD ) );
        AddShape( new Box( Vec2(), Lib::HEIGHT / 2, M_SEVEN, 0xcc66ffff, 0, ENEMY | SHIELD ) );
        AddShape( new Box( Vec2(), Lib::HEIGHT / 2, M_FOUR, 0xcc66ffff, 0, ENEMY | SHIELD ) );
    }
...
, _dir( swap ? M_ONE : -M_ONE, 0 )
{
    ...
    AddShape( new Box( Vec2( 0, off - 20 - Lib::HEIGHT / 2 ), M_TEN, Lib::HEIGHT / 2, 0xcc66ffff, 0, ENEMY | SHIELD ) );
    AddShape( new Box( Vec2( 0, off + 20 + Lib::HEIGHT / 2 ), M_TEN, Lib::HEIGHT / 2, 0xcc66ffff, 0, ENEMY | SHIELD ) );

    AddShape( new Box( Vec2( 0, off - 20 - Lib::HEIGHT / 2 ), M_SEVEN, Lib::HEIGHT / 2, 0xcc66ffff, 0, 0 ) );
    AddShape( new Box( Vec2( 0, off + 20 + Lib::HEIGHT / 2 ), M_SEVEN, Lib::HEIGHT / 2, 0xcc66ffff, 0, 0 ) );
    
    AddShape( new Box( Vec2( 0, off - 20 - Lib::HEIGHT / 2 ), M_FOUR, Lib::HEIGHT / 2, 0xcc66ffff, 0, 0 ) );
    AddShape( new Box( Vec2( 0, off + 20 + Lib::HEIGHT / 2 ), M_FOUR, Lib::HEIGHT / 2, 0xcc66ffff, 0, 0 ) );
}
...
void GhostMine::Update()
{
    ...
    for ( unsigned int i = 0; i < s.size(); i++ ) {
        if ( s[ i ] == _ghost ) {
            Enemy* e = ( GetLib().RandInt( 6 ) == 0 || ( _ghost->IsHPLow() && GetLib().RandInt( 5 ) == 0 ) ) ? new BigFollow( GetPosition(), false ) : new Follow( GetPosition() );
            e->SetScore( 0 );
            Spawn( e );
            Damage( 1, false, 0 );
            break;
...
void DeathArm::Update()
{
    ...
    if ( _attacking ) {
        ...
        if ( _timer < DeathRayBoss::ARM_ATIMER / 4 ) {
            ...
            Vec2 d = _boss->GetPosition() + Vec2( M_ONE * 80, _top ? M_ONE * 80 : -M_ONE * 80 ) - GetPosition();
    ...
    SetPosition( _boss->GetPosition() + Vec2( M_ONE * 80, _top ? M_ONE * 80 : -M_ONE * 80 ) );
...
, _dTimer( 0 )
{
    AddShape( new Polygon( Vec2(), 10, 4, colour, 0, ENEMY | SHIELD | VULNSHIELD ) );
    SetBoundingWidth( 22 );
    SetScore( 0 );
}
