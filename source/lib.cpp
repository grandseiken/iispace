...
Lib::SaveData Lib::LoadSaveData()
{
    ...
    if ( findTotal != total || findTotal % 17 != t17 || findTotal % 701 != t701 || findTotal % 1171 != t1171 || findTotal % 1777 != t1777 ) {
        r.clear();
        for ( int i = 0; i < 4 * PLAYERS + 1; i++ ) {
            r.push_back( HighScoreList() );
            for ( unsigned int j = 0; j < ( i != PLAYERS ? NUM_HIGH_SCORES : PLAYERS ); j++ )
...
void Lib::SaveSaveData( const SaveData& version2 )
{
    ...
    for ( int i = 0; i < 4 * PLAYERS + 1; ++i ) {
        for ( unsigned int j = 0; j < ( i != PLAYERS ? NUM_HIGH_SCORES : PLAYERS ); ++j ) {
            if ( i < int( version2._highScores.size() ) && j < version2._highScores[ i ].size() )
                total += version2._highScores[ i ][ j ].second;
        }
    }

    std::stringstream out;
    out << "WiiSPACE v1.3\n" << version2._bossesKilled << " " << version2._hardModeBossesKilled << " " << total
        << " " << ( total % 17 ) << " " << ( total % 701 ) << " " << ( total % 1171 ) << " " << ( total % 1777 ) << "\n";

    for ( int i = 0; i < 4 * PLAYERS + 1; ++i ) {
        for ( unsigned int j = 0; j < ( i != PLAYERS ? NUM_HIGH_SCORES : PLAYERS ); ++j ) {
            if ( i < int( version2._highScores.size() ) && j < version2._highScores[ i ].size() )
                out << version2._highScores[ i ][ j ].second << " " << version2._highScores[ i ][ j ].first << "\n";
            else
                out << "0\n";
        }
    }
...
void Lib::SaveSaveSettings( const Settings& settings )
{
    std::ofstream out;
    out.open( SettingsPath().c_str() );
    out << SETTINGS_DISABLEBACKGROUND << " " << ( settings._disableBackground ? "1" : "0" ) << "\n" << SETTINGS_HUDCORRECTION << " " << settings._hudCorrection << "\n" <<
           SETTINGS_WINDOWED << " " << ( settings._windowed ? "1" : "0" ) << "\n" << SETTINGS_VOLUME << " " << z_int( settings._volume );
    out.close();
}

void Lib::StartRecording( int players, bool canFaceSecretBoss, bool isBossMode, bool isHardMode, bool isFastMode, bool isWhatMode )
{
    NewGame();
    ...
    z_srand( _recordSeed );
    _record << "WiiSPACE v1.3 replay\n" << players << " " << canFaceSecretBoss << " " << isBossMode << " " << isHardMode << " " << isFastMode << " " << isWhatMode << " " << _recordSeed << "\n";
    _recording = true;
}

void Lib::EndRecording( const std::string& name, long score, int players, bool bossMode, bool hardMode, bool fastMode, bool whatMode )
{
    if ( !_recording )
        return;
    _recording = false;
    std::stringstream ss;
    ss << "replays/" << _recordSeed << "_" << players << "p_" << ( bossMode ? "bossmode_" : hardMode ? "hardmode_" : fastMode ? "fastmode_" : whatMode ? "w-hatmode_" : "" ) << name << "_" << score << ".wrp";
    std::ofstream file;
...
const Lib::Recording& Lib::PlayRecording( const std::string& path )
{
    ...
    std::stringstream decrypted( temp, std::ios::in | std::ios::out | std::ios::binary );
...
Colour Lib::RgbToHsl( Colour rgb ) const
    ...
    if ( r == v )
          ...
          h = ( r == m ? 3.0 + g2 : 5.0 - r2 );
    h /= 6.0;
    return ( ( int( 0.5 + h * 255 ) & 0xff ) << 24 ) | ( ( int( 0.5 + s * 255 ) & 0xff ) << 16 ) | ( ( int( 0.5 + l * 255 ) & 0xff ) << 8 );
}
...
Colour Lib::HslToRgb( Colour hsl ) const
{
    ...
    return ( ( int( 0.5 + r * 255 ) & 0xff ) << 24 ) | ( ( int( 0.5 + g * 255 ) & 0xff ) << 16 ) | ( ( int( 0.5 + b * 255 ) & 0xff ) << 8 );
}
