...
#include <vector>
#include <string>
#include "../ois-mod/includes/OISJoyStick.h"
#include "../ois-mod/includes/OISInputManager.h"
#include "../ois-mod/includes/OISForceFeedback.h"
#include "gamepadcfg.h"
...
public:
    ...
    std::pair< std::pair< int, bool >, std::pair< int, bool > > axis()
    {
        ...
        std::pair< std::pair< int, bool >, std::pair< int, bool > > axes( std::make_pair( _axis1, _axis1r ), std::make_pair( _axis2, _axis2r ) );
        _axis1 = -1;
        _axis2 = -1;
        return axes;
    }
    ...
    {
        if ( ( _axis1 == -1 || _axis1 != axis ) && ( arg.state.mAxes[ axis ].abs > 20000 || arg.state.mAxes[ axis ].abs < -20000 ) ) {
            if ( _axis1 == -1 ) {
                _axis1 = axis;
...
int assignStart( Handler& handler, OIS::JoyStick** pads, int& i, const std::string& name, PadConfig::Buttons& buttons, std::string& devName, const std::vector< int >& done )
{
    ...
    while ( true ) {
        ...
        if ( t >= 0 ) {
            ...
            if ( !found ) {
                ...
                std::cout << "\tAssigned button " << t << " to " << name << " on " << devName << "\n";
                return t;
            }
        }
    }
...
void assign( Handler& handler, OIS::JoyStick** pads, int p, int i, const std::string& name, PadConfig::Sticks& sticks, PadConfig::Buttons& dpads, int start, const std::string& devName )
{
    std::cout << "Tilt any [" << name << "] sticks FIRST UPWARDS AND THEN TO THE RIGHT, and press any [" << name << "] D-pads, on gamepad for PLAYER " << i + 1 << "\n(or press [START] to continue):\n";
    handler.clear();
    while ( true ) {
		pads[ p ]->capture();
        int t = handler.pov();
        if ( t >= 0 ) {
            ...
            if ( !found ) {
                dpads.push_back( t );
                std::cout << "\tAssigned D-pad " << t << " to " << name << " on " << devName << "\n";
            }
        }
        ...
        if ( u.first.first >= 0 && u.second.first >= 0 && u.first.first != u.second.first ) {
            ...
            for ( std::size_t j = 0; j < sticks.size(); ++j ) {
                if ( sticks[ j ]._axis1 == u.first.first || sticks[ j ]._axis2 == u.second.first || sticks[ j ]._axis1 == u.second.first || sticks[ j ]._axis2 == u.first.first )
                    found = true;
            }
            if ( !found ) {
                ...
                std::cout << "\tAssigned stick " << stick._axis1 << ", " << stick._axis2 << " to " << name << " on " << devName << "\n";
            }
        }
...
void assign( Handler& handler, OIS::JoyStick** pads, int p, int i, const std::string& name, PadConfig::Buttons& buttons, int start, const std::string& devName )
{
...
int main( int argc, char** argv )
{
    ...
    for ( int i = 0; i < n; ++i ) {
        handler.clear();
        int p = i;
        int start = assignStart( handler, pads, p, "START/MENU", config[ i ]._startButtons, config[ i ]._name, done );
        done.push_back( p );

        assign( handler, pads, p, i, "MOVEMENT", config[ i ]._moveSticks, config[ i ]._moveDpads, start, config[ i ]._name );
        assign( handler, pads, p, i, "AIMING", config[ i ]._aimSticks, config[ i ]._aimDpads, start, config[ i ]._name );
