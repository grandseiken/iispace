...
public:
    ...
    virtual void RenderLine( const Vec2f& a, const Vec2f& b, Colour c ) const = 0;
    virtual void RenderText( const Vec2f& v, const std::string& text, Colour c ) const = 0;
    virtual void RenderRect( const Vec2f& low, const Vec2f& hi, Colour c, int lineWidth = 0 ) const = 0;
    virtual void Render() const = 0;

    virtual void Rumble( int player, int time ) = 0;
    virtual void StopRumble() = 0;
    virtual bool PlaySound( Sound sound, float volume = 1.f, float pan = 0.f, float repitch = 0.f ) = 0;

    virtual void TakeScreenShot() = 0;

    // Recording
    //------------------------------
    void StartRecording( int players, bool canFaceSecretBoss, bool isBossMode, bool isHardMode, bool isFastMode, bool isWhatMode );
    void Record( Vec2 velocity, Vec2 target, int keys );
    void EndRecording( const std::string& name, long score, int players, bool bossMode, bool hardMode, bool fastMode, bool whatMode );
