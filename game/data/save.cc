...
result<savegame> read_savegame(std::span<const std::uint8_t> bytes) {
  ...
  get_mode_table(proto.normal(), data.high_scores.normal);
  get_mode_table(proto.hard(), data.high_scores.hard);
  get_mode_table(proto.fast(), data.high_scores.fast);
  get_mode_table(proto.what(), data.high_scores.what);
  std::size_t players = 0;
  for (const auto& s : proto.boss().score()) {
    data.high_scores.boss[players].name = s.name();
    data.high_scores.boss[players].score = s.score();
    ++players;
  }
...
result<std::vector<std::uint8_t>> write_savegame(const savegame& data) {
  ...
  put_mode_table(data.high_scores.normal, *proto.mutable_normal());
  put_mode_table(data.high_scores.hard, *proto.mutable_hard());
  put_mode_table(data.high_scores.fast, *proto.mutable_fast());
  put_mode_table(data.high_scores.what, *proto.mutable_what());
  for (const auto& s : data.high_scores.boss) {
