#ifndef II_GAME_LOGIC_GEOMETRY_LEGACY_BOX_H
...
struct box_data : shape_data_base {
  ...
  iterate(iterate_lines_t, const Transform auto& t, const LineFunction auto& f) const {
    ...
    std::invoke(f, a, b, colour, 1.f);
    std::invoke(f, b, c, colour, 1.f);
    std::invoke(f, c, d, colour, 1.f);
    std::invoke(f, d, a, colour, 1.f);
  }
