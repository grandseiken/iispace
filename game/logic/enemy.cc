...
: Enemy(position, SHIP_NONE, hp), _timer(0), _target(0) {
  add_shape(new Polygon(vec2(), radius, 4, 0x9933ffff, 0, DANGEROUS | VULNERABLE));
  set_score(15);
...
: Enemy(position, SHIP_NONE, 2), _move(false), _timer(CHASER_TIME), _dir() {
  add_shape(
      new Polygon(vec2(), 10, 4, 0x3399ffff, 0, DANGEROUS | VULNERABLE, Polygon::T::POLYGRAM));
  set_score(30);
  set_bounding_width(10);
  set_destroy_sound(Lib::SOUND_ENEMY_SHATTER);
  set_enemy_value(2);
...
void Chaser::update() {
  ...
  if (_timer <= 0) {
    ...
    _move = !_move;
    if (_move) {
      _dir = CHASER_SPEED * (nearest_player()->shape().centre - shape().centre).normalised();
    }
  }
  if (_move) {
    move(_dir);
...
: Enemy(position, SHIP_WALL, 4), _dir(), _timer(z::rand_int(80) + 40) {
...
: Enemy(position, SHIP_WALL, 10), _dir(0, 1), _timer(0), _rotate(false), _rdir(rdir) {
...
, _power(power) {
  ...
  add_shape(
      new Polygon(vec2(0, 0), 14, 8, power ? 0x33cc99ff : 0x006633ff, 0, DANGEROUS | VULNERABLE));
...
void Shielder::update() {
  ...
  if (_rotate) {
    ...
    if (is_on_screen() && _power && _timer % SHIELDER_TIMER == SHIELDER_TIMER / 2) {
...
, _spinning(false) {
  add_shape(
      new Polygon(vec2(24, 0), 12, 6, 0xcc33ccff, 0, DANGEROUS | VULNERABLE, Polygon::T::POLYGRAM));
  add_shape(new Polygon(vec2(-24, 0), 12, 6, 0xcc33ccff, 0, DANGEROUS | VULNERABLE,
                        Polygon::T::POLYGRAM));
  add_shape(new Line(vec2(0, 0), vec2(24, 0), vec2(-24, 0), 0xcc33ccff));
  if (power) {
    add_shape(new Polygon(vec2(24, 0), 16, 6, 0xcc33ccff, 0, 0, Polygon::T::POLYSTAR));
    add_shape(new Polygon(vec2(-24, 0), 16, 6, 0xcc33ccff, 0, 0, Polygon::T::POLYSTAR));
  }
...
: Enemy(position, SHIP_WALL, 0), _dir(velocity) {
...
void BossShot::update() {
  move(_dir);
  vec2 p = shape().centre;
  if ((p.x < -10 && _dir.x < 0) || (p.x > Lib::WIDTH + 10 && _dir.x > 0) ||
