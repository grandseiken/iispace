...
void destruct_lines(EmitHandle& e, const geom::resolve_result& r, const fvec2& source,
  ...
  for (const auto& entry : r.entries) {
    if (const auto* d = std::get_if<geom::box_data>(&entry.data)) {
      ...
      if (d->line.style != render::ngon_style::kPolygram) {
        for (std::uint32_t i = 0;
             d->dimensions.sides >= 2 && i < d->dimensions.sides && i < d->dimensions.segments;
             ++i) {
          handle_line(vertex(i),
                      d->line.style == render::ngon_style::kPolygon ? vertex(i + 1) : entry.t.v,
                      d->line.colour0);
        }
      } else {
...
void render_shape(std::vector<render::shape>& output, const geom::resolve_result& r, float z_index,
