...
void GlobalData::post_update(ecs::handle h, SimInterface& sim) {
  for (auto it = fireworks.begin(); it != fireworks.end();) {
    ...
    auto explode = [&](const fvec2& v, const cvec4& c, std::uint32_t time) {
      ...
      e.explosion(v + fvec2{8.f, 0.f}, c, time);
      e.explosion(v + fvec2{0.f, 8.f}, c, time);
    ...
    explode(to_float(it->position), cvec4{1.f}, 16);
    explode(to_float(it->position), it->colour, 32);
    explode(to_float(it->position), cvec4{1.f}, 64);
